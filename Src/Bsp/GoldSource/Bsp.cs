...
namespace BspLib.Bsp.GoldSource
{
    public static class Bsp
    {
        ...
        public static bool LoadVisualsFromData(BspFile bsp, Vector3[] vertices, Edge[] edges, int[] surfaceEdges, Face[] faces, string[] usedTextures, Dictionary<string, Vector2> textureDimensions, TextureInfo[] textureInfo, ModelData[] modelDatas)
        {
            if (bsp == null)
                throw new ArgumentNullException("bsp", "Target BSP is null.");
            if (modelDatas.Length == 0)
                throw new ArgumentOutOfRangeException("modelDatas", "No enought models. At least one model is needed.");

            var models = new BspLib.Bsp.BspFile.Model[modelDatas.Length];
        ...
        public static bool LoadVisualsFromDirectory(BspFile bsp, string directory_path, string file_name, string[] extensions = null)
        {
            ...
            if (extensions.Length != (int)Lumps._Count)
                throw new ArgumentOutOfRangeException("extensions", "Invalid number of extensions. Must be same as number of lumps.");
        ...
        public static bool LoadCollidersFromData(BspFile bsp, Vector3[] vertices, Edge[] edges, short[] markSurfaces, Leaf[] leaves, ClipNode[] clipNodes)
        {
            if (bsp == null)
                throw new ArgumentNullException("bsp", "Target BSP is null.");

			Debug.Log("Load Colliders From Data");

            var mesh = ClipNodesToMesh(vertices, edges, markSurfaces, leaves, clipNodes);
            bsp.Colliders.Add(mesh);
            return true;
        }
        ...
        public static bool LoadPackedTexturesFromData(BspFile bsp, List<TextureByteIndexPaletteWithMipmaps> textures)
        {
			...
			foreach(var t in textures)
				bsp.PackedGoldSourceTextures.Add(t);
            return true;
        }
        ...
        public static bool LoadAllFromBspStream(BspFile bsp, BspLib.Bsp.BspFile.LoadFlags flags, Stream bsp_stream)
        {
            if (bsp == null)
                throw new ArgumentNullException("bsp", "Target BSP is null.");
            ...
            if ((flags & BspLib.Bsp.BspFile.LoadFlags.Visuals) != 0)
            {
                if (vertices == null)
                {
                    bsp_stream.Position = file_start + header[(int)Lumps.Vertices].Offset;
			...
			Debug.Log("Before -  Loading colliders");
            if ((flags & BspLib.Bsp.BspFile.LoadFlags.Colliders) != 0)
            {
				Debug.Log("Loading colliders");
            ...
            if ((flags & BspLib.Bsp.BspFile.LoadFlags.Entities) != 0)
            {
                bsp_stream.Position = file_start + header[(int)Lumps.Entities].Offset;
                var entities = ReadEntityLump(bsp_stream, header[(int)Lumps.Entities].Size);
            ...
            if ((flags & BspLib.Bsp.BspFile.LoadFlags.PackedTextures) != 0)
            {
                bsp_stream.Position = file_start + header[(int)Lumps.Textures].Offset;
                var tuple = ReadTextureLump(bsp_stream, header[(int)Lumps.Textures].Size);
        ...
        public static bool LoadAllFromFile(BspFile bsp, BspLib.Bsp.BspFile.LoadFlags flags, string bsp_path)
        {
            if (bsp == null)
                throw new ArgumentNullException("bsp", "Target BSP is null.");
        ...
        public static bool LoadAllFromFiles(BspFile bsp, BspLib.Bsp.BspFile.LoadFlags flags, string[] file_paths)
        {
            if (bsp == null)
                throw new ArgumentNullException("bsp", "Target BSP is null.");
        ...
        public static bool LoadAllFromDirectory(BspFile bsp, BspLib.Bsp.BspFile.LoadFlags flags, string directory_path, string[] file_names)
        {
            if (bsp == null)
                throw new ArgumentNullException("bsp", "Target BSP is null.");
        ...
        public static bool LoadAllFromDirectory(BspFile bsp, BspLib.Bsp.BspFile.LoadFlags flags, string directory_path, string file_name, string[] extensions = null)
        {
            if (bsp == null)
                throw new ArgumentNullException("bsp", "Target BSP is null.");
		...
		public static Texture2D CreateLightingBitmap(byte[] lightmap, int offset, int width, int height)
        {
        ...
        public class BspNode
        {
            public BspNode(uint planeLumpIndex, short[] children, short[] bbMin, short[] bbMax, ushort firstFace, ushort faceCount) : this(planeLumpIndex, children[0], children[1], bbMin, bbMax, firstFace, faceCount)
            {
                if (children.Length != 2)
                    throw new ArgumentOutOfRangeException("children");
            }
            ...
            {
                if (bbMin.Length != 3)
                    throw new ArgumentOutOfRangeException("bbMin");
                if (bbMax.Length != 3)
                    throw new ArgumentOutOfRangeException("bbMax");
        ...
        public static void CreateCollisionPolygons(IList<Vector3> points, IList<BspLib.Bsp.BspFile.Polygon> polygons, int firstMarkSurface, int markSurfaceCount, Vector3[] vertices, Edge[] edges, short[] markSurfaces)
		{
			Debug.LogFormat("MarkSurface count: {0}", markSurfaceCount);
            ...
            polygons.Add(new BspLib.Bsp.BspFile.Polygon("clip", vertex_indices.ToArray()));
        }

        public static Tuple<Vector3[], BspLib.Bsp.BspFile.Polygon[]> ClipNodesToPolygons(Vector3[] vertices, Edge[] edges, short[] markSurfaces, Leaf[] leaves, ClipNode[] clipNodes)
        {
            ...
            var polygons = new List<BspLib.Bsp.BspFile.Polygon>();
            ...
            return new Tuple<Vector3[], BspLib.Bsp.BspFile.Polygon[]>(points.ToArray(), polygons.ToArray());
        }

        public static Tuple<Vector3[], uint[]> ClipNodesToTriangles(Vector3[] vertices, Edge[] edges, short[] markSurfaces, Leaf[] leaves, ClipNode[] clipNodes)
        {
            ...
            return new Tuple<Vector3[], uint[]>(tuple.Item1, triangles.ToArray());
        }

        public static Mesh ClipNodesToMesh(Vector3[] vertices, Edge[] edges, short[] markSurfaces, Leaf[] leaves, ClipNode[] clipNodes)
        {
			...
			mesh.SetVertices(new List<Vector3>(tuple.Item1));

			var indices = new int[tuple.Item2.Length];
			for(int i = 0; i < indices.Length; i++)
				indices[i] = (int)tuple.Item2[i];

			mesh.SetIndices(indices, MeshTopology.Triangles, 0);
			return mesh;
        }

        public static void CreateVisualPolygons(IList<Point> points, IList<BspLib.Bsp.BspFile.Polygon> polygons, int firstFace, int faceCount, Vector3[] vertices, Edge[] edges, int[] surface_edges, Face[] faces, TextureInfo[] texture_info, string[] textures_used, Dictionary<string, Vector2> texture_dimensions)
        {
            for (int f = 0; f < faceCount; f++)
            {
                ...
                polygons.Add(new BspLib.Bsp.BspFile.Polygon(textureName, vertex_indices.ToArray()));
            }
        }

        public static BspLib.Bsp.BspFile.Model ModelDataToPolygon(ModelData model, Vector3[] vertices, Edge[] edges, int[] surface_edges, Face[] faces, TextureInfo[] texture_info, string[] textures_used, Dictionary<string, Vector2> texture_dimensions)
        {
            ...
            var points_position = new List<Vector3>();
            var points_uv = new List<Vector2>();
            ...
            var sorted_polygons = BspLib.Bsp.BspFile.Model.GroupPolygonsByTexture(polygons);
            var triangles = BspLib.Bsp.BspFile.Model.PolygonsToTrianglesPoints(sorted_polygons);
            return new BspLib.Bsp.BspFile.Model(triangles, points_position.ToArray(), points_uv.ToArray());
        }

        public class Point
        {
            public Point(Vector3 position, Vector2 uv)
            {
        ...
        private static MemoryStream CreateStream(BinaryReader reader, Lump lump)
		{
			return CreateStream(reader, lump.Offset, lump.Size);
		}
        ...
        public static Tuple<List<TextureByteIndexPaletteWithMipmaps>, string[], Dictionary<string, Vector2>> ReadTextureLump(Stream stream, long length = -1, bool loadPackedTextures = true)
        {
            ...
            for (int i = 0; i < count; i++)
            {
				...
				stream.Position = texture_offset;
        ...
        {
            public TextureInfo(Vector3 s, float s_shift, Vector3 t, float t_shift, uint textureID, TexInfoFlags flags)
            {
                this.S = s;
                this.SShift = s_shift;
            ...
            public Vector2 GetTexelUV(Vector3 position, Vector2 textureSize)
            {
                return new Vector2(GetTexelU(position, textureSize), GetTexelV(position, textureSize));
            }

            public float GetTexelU(Vector3 position, Vector2 textureSize)
            {
				return (S.x * position.x + S.y * position.y + S.z * position.z + SShift) / textureSize.x;
            }

            public float GetTexelV(Vector3 position, Vector2 textureSize)
            {
        ...
        public static Vector3[] ReadVerticesLump(Stream stream, long length = -1)
        {
            var reader = new BinaryReader(stream);
			...
			int count = (int)(length / (3 * sizeof(float)));
        ...
        {
            public ModelData(Vector3 mins, Vector3 maxs, Vector3 origin, int[] headnodes, int visLeafs, int firstFace, int faceCount)
            {
                this.Mins = mins;
                this.Maxs = maxs;
        ...
        {
            public PlaneData(Vector3 normal, float distance, PlaneType type)
            {
                this.Normal = normal;
        ...
        public static void SplitBspIntoFiles(string bspPath, string folder)
		{
			SplitBspIntoFiles(bspPath, folder, DefaultExtensions);
		}
        ...
        public static void SplitBspIntoFiles(BinaryReader reader, string name, string folder)
		{
			SplitBspIntoFiles(reader, name, folder, DefaultExtensions);
		}
        ...
        public static BspLib.Wad.Texture[] GetPackedTextures(string path)
        {
            if (!File.Exists(path))
        ...
        public static BspLib.Wad.Texture[] GetPackedTextures(BinaryReader reader)
        {
            // Version
            {
                var version = reader.ReadUInt32();
